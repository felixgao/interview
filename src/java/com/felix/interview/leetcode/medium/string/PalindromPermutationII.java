...
public class PalindromPermutationII {
    public List<String> generatePalindromes(String s) {
        ...
        if(!isPalindromMap(characterIntegerMap)) return result;

        return result;
    }

    boolean isPalindromMap(Map<Character, Integer> characterIntegerMap) {
        long c = characterIntegerMap.values().stream().filter(i -> i % 2 == 1).mapToInt(i -> i).count();
    ...
    void permutateCharacters(List<Character> allowedCharacters, boolean[] used, String mid, StringBuilder sb, List<String> result){
        ...
        for(int i =0; i < allowedCharacters.size(); i++){
            ...
            used[i] = true; sb.append(allowedCharacters.get(i));
